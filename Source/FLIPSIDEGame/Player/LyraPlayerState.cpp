...
void ALyraPlayerState::OnExperienceLoaded(const UFlipsideExperienceDefinition* /*CurrentExperience*/)
{
	if (ALyraGameMode* LyraGameMode = GetWorld()->GetAuthGameMode<ALyraGameMode>())
	{
		...
		{
			UE_LOG(LogFlipside, Error, TEXT("ALyraPlayerState::OnExperienceLoaded(): Unable to find PawnData to initialize player state [%s]!"), *GetNameSafe(this));
		}
	}
}
...
void ALyraPlayerState::SetPawnData(const ULyraPawnData* InPawnData)
{
	...
	if (PawnData)
	{
		UE_LOG(LogFlipside, Error, TEXT("Trying to set PawnData [%s] on player state [%s] that already has valid PawnData [%s]."), *GetNameSafe(InPawnData), *GetNameSafe(this), *GetNameSafe(PawnData));
		return;
	}
...
void ALyraPlayerState::SetGenericTeamId(const FGenericTeamId& NewTeamID)
{
	...
	{
		UE_LOG(LogFlipsideTeams, Error, TEXT("Cannot set team for %s on non-authority"), *GetPathName(this));
	}
}
