...
void ALyraPlayerController::ServerCheat_Implementation(const FString& Msg)
{
	...
	if (CheatManager)
	{
		UE_LOG(LogFlipside, Warning, TEXT("ServerCheat: %s"), *Msg);
...
void ALyraPlayerController::ServerCheatAll_Implementation(const FString& Msg)
{
	...
	if (CheatManager)
	{
		UE_LOG(LogFlipside, Warning, TEXT("ServerCheatAll: %s"), *Msg);
...
bool ALyraPlayerController::GetIsAutoRunning() const
{
	...
	{
		bIsAutoRunning = LyraASC->GetTagCount(FlipsideGameplayTags::Status_AutoRunning) > 0;
	}
...
void ALyraPlayerController::OnStartAutoRun()
{
	if (ULyraAbilitySystemComponent* LyraASC = GetLyraAbilitySystemComponent())
	{
		LyraASC->SetLooseGameplayTagCount(FlipsideGameplayTags::Status_AutoRunning, 1);
		K2_OnStartAutoRun();
	}	
}
...
void ALyraPlayerController::OnEndAutoRun()
{
	if (ULyraAbilitySystemComponent* LyraASC = GetLyraAbilitySystemComponent())
	{
		LyraASC->SetLooseGameplayTagCount(FlipsideGameplayTags::Status_AutoRunning, 0);
		K2_OnEndAutoRun();
	}
}
...
{
	UE_LOG(LogFlipsideTeams, Error, TEXT("You can't set the team ID on a player controller (%s); it's driven by the associated player state"), *GetPathNameSafe(this));
}
