...
void ULyraPawnExtensionComponent::SetPawnData(const ULyraPawnData* InPawnData)
{
	...
	if (PawnData)
	{
		UE_LOG(LogFlipside, Error, TEXT("Trying to set PawnData [%s] on pawn [%s] that already has valid PawnData [%s]."), *GetNameSafe(InPawnData), *GetNameSafe(Pawn), *GetNameSafe(PawnData));
		return;
	}
...
void ULyraPawnExtensionComponent::InitializeAbilitySystem(ULyraAbilitySystemComponent* InASC, AActor* InOwnerActor)
{
	...
	UE_LOG(LogFlipside, Verbose, TEXT("Setting up ASC [%s] on pawn [%s] owner [%s], existing [%s] "), *GetNameSafe(InASC), *GetNameSafe(Pawn), *GetNameSafe(InOwnerActor), *GetNameSafe(ExistingAvatar));

	if ((ExistingAvatar != nullptr) && (ExistingAvatar != Pawn))
	{
		UE_LOG(LogFlipside, Log, TEXT("Existing avatar (authority=%d)"), ExistingAvatar->HasAuthority() ? 1 : 0);
...
void ULyraPawnExtensionComponent::UninitializeAbilitySystem()
{
	...
	if (AbilitySystemComponent->GetAvatarActor() == GetOwner())
	{
		...
		AbilityTypesToIgnore.AddTag(FlipsideGameplayTags::Ability_Behavior_SurvivesDeath);
...
bool ULyraPawnExtensionComponent::CanChangeInitState(UGameFrameworkComponentManager* Manager, FGameplayTag CurrentState, FGameplayTag DesiredState) const
{
	...
	if (CurrentState == FlipsideGameplayTags::InitState_Spawned && DesiredState == FlipsideGameplayTags::InitState_DataAvailable)
	{
	...
	else if (CurrentState == FlipsideGameplayTags::InitState_DataAvailable && DesiredState == FlipsideGameplayTags::InitState_DataInitialized)
	{
		...
		return Manager->HaveAllFeaturesReachedInitState(Pawn, FlipsideGameplayTags::InitState_DataAvailable);
	}
...
{
	if (DesiredState == FlipsideGameplayTags::InitState_DataInitialized)
	{
...
void ULyraPawnExtensionComponent::OnActorInitStateChanged(const FActorInitStateChangedParams& Params)
{
	...
	{
		if (Params.FeatureState == FlipsideGameplayTags::InitState_DataAvailable)
		{
			CheckDefaultInitialization();
		}
	}
