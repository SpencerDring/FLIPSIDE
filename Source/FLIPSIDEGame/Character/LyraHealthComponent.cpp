...
#include "System/FLIPSIDEGameData.h"
...
void ULyraHealthComponent::InitializeWithAbilitySystem(ULyraAbilitySystemComponent* InASC)
{
	...
	{
		UE_LOG(LogFlipside, Error, TEXT("LyraHealthComponent: Health component for owner [%s] has already been initialized with an ability system."), *GetNameSafe(Owner));
		return;
	}
	...
	if (!AbilitySystemComponent)
	{
		UE_LOG(LogFlipside, Error, TEXT("LyraHealthComponent: Cannot initialize health component for owner [%s] with NULL ability system."), *GetNameSafe(Owner));
		return;
	}
	...
	if (!HealthSet)
	{
		UE_LOG(LogFlipside, Error, TEXT("LyraHealthComponent: Cannot initialize health component for owner [%s] with NULL health set on the ability system."), *GetNameSafe(Owner));
		return;
	}
...
void ULyraHealthComponent::ClearGameplayTags()
{
	if (AbilitySystemComponent)
	{
		AbilitySystemComponent->SetLooseGameplayTagCount(FlipsideGameplayTags::Status_Death_Dying, 0);
		AbilitySystemComponent->SetLooseGameplayTagCount(FlipsideGameplayTags::Status_Death_Dead, 0);
	}
}
...
void ULyraHealthComponent::HandleOutOfHealth(AActor* DamageInstigator, AActor* DamageCauser, const FGameplayEffectSpec& DamageEffectSpec, float DamageMagnitude)
{
	...
	if (AbilitySystemComponent)
	{
		// Send the "GameplayEvent.Death" gameplay event through the owner's ability system.  This can be used to trigger a death gameplay ability.
		{
			...
			Payload.EventTag = FlipsideGameplayTags::GameplayEvent_Death;
...
void ULyraHealthComponent::OnRep_DeathState(ELyraDeathState OldDeathState)
{
	...
	if (OldDeathState > NewDeathState)
	{
		...
		UE_LOG(LogFlipside, Warning, TEXT("LyraHealthComponent: Predicted past server death state [%d] -> [%d] for owner [%s]."), (uint8)OldDeathState, (uint8)NewDeathState, *GetNameSafe(GetOwner()));
		return;
	}
	...
	if (OldDeathState == ELyraDeathState::NotDead)
	{
		...
		{
			UE_LOG(LogFlipside, Error, TEXT("LyraHealthComponent: Invalid death transition [%d] -> [%d] for owner [%s]."), (uint8)OldDeathState, (uint8)NewDeathState, *GetNameSafe(GetOwner()));
		}
	}
...
void ULyraHealthComponent::StartDeath()
{
	...
	if (AbilitySystemComponent)
	{
		AbilitySystemComponent->SetLooseGameplayTagCount(FlipsideGameplayTags::Status_Death_Dying, 1);
	}
...
void ULyraHealthComponent::FinishDeath()
{
	...
	if (AbilitySystemComponent)
	{
		AbilitySystemComponent->SetLooseGameplayTagCount(FlipsideGameplayTags::Status_Death_Dead, 1);
	}
...
void ULyraHealthComponent::DamageSelfDestruct(bool bFellOutOfWorld)
{
	...
	{
		const TSubclassOf<UGameplayEffect> DamageGE = UFlipsideAssetManager::GetSubclass(UFlipsideGameData::Get().DamageGameplayEffect_SetByCaller);
		if (!DamageGE)
		{
			UE_LOG(LogFlipside, Error, TEXT("LyraHealthComponent: DamageSelfDestruct failed for owner [%s]. Unable to find gameplay effect [%s]."), *GetNameSafe(GetOwner()), *UFlipsideGameData::Get().DamageGameplayEffect_SetByCaller.GetAssetName());
			return;
		}
		...
		if (!Spec)
		{
			UE_LOG(LogFlipside, Error, TEXT("LyraHealthComponent: DamageSelfDestruct failed for owner [%s]. Unable to make outgoing spec for [%s]."), *GetNameSafe(GetOwner()), *GetNameSafe(DamageGE));
			return;
		}
		...
		Spec->SetSetByCallerMagnitude(FlipsideGameplayTags::SetByCaller_Damage, DamageAmount);
