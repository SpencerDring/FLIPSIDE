...
#include "System/FLIPSIDEAssetManager.h"
...
#include "GameModes/FlipsideExperienceDefinition.h"
#include "GameModes/LyraExperienceManagerComponent.h"
#include "GameModes/FLIPSIDEUserFacingExperienceDefinition.h"
...
const ULyraPawnData* ALyraGameMode::GetPawnDataForController(const AController* InController) const
{
	...
	{
		const UFlipsideExperienceDefinition* Experience = ExperienceComponent->GetCurrentExperienceChecked();
		...
		return UFlipsideAssetManager::Get().GetDefaultPawnData();
	}
...
void ALyraGameMode::HandleMatchAssignmentIfNotExpectingOne()
{
	...
	if (!ExperienceId.IsValid() && UGameplayStatics::HasOption(OptionsString, TEXT("Experience")))
	{
		...
		ExperienceId = FPrimaryAssetId(FPrimaryAssetType(UFlipsideExperienceDefinition::StaticClass()->GetFName()), FName(*ExperienceFromOptions));
	...
	if (!ExperienceId.IsValid())
	{
		...
		if (FParse::Value(FCommandLine::Get(), TEXT("Experience="), ExperienceFromCommandLine))
		{
			...
			{
				ExperienceId = FPrimaryAssetId(FPrimaryAssetType(UFlipsideExperienceDefinition::StaticClass()->GetFName()), FName(*ExperienceFromCommandLine));
			}
	...
	UFlipsideAssetManager& AssetManager = UFlipsideAssetManager::Get();
	FAssetData Dummy;
	if (ExperienceId.IsValid() && !AssetManager.GetPrimaryAssetData(ExperienceId, /*out*/ Dummy))
	{
		UE_LOG(LogFlipsideExperience, Error, TEXT("EXPERIENCE: Wanted to use %s but couldn't find it, falling back to the default)"), *ExperienceId.ToString());
	...
	if (!ExperienceId.IsValid())
	{
		...
		ExperienceId = FPrimaryAssetId(FPrimaryAssetType("FlipsideExperienceDefinition"), FName("B_LyraDefaultExperience"));
...
{
	FPrimaryAssetType UserExperienceType = UFlipsideUserFacingExperienceDefinition::StaticClass()->GetFName();
	...
	UFlipsideUserFacingExperienceDefinition* FoundExperience = nullptr;
	UFlipsideUserFacingExperienceDefinition* DefaultExperience = nullptr;

	for (UObject* Object : UserExperiences)
	{
		UFlipsideUserFacingExperienceDefinition* UserExperience = Cast<UFlipsideUserFacingExperienceDefinition>(Object);
...
void ALyraGameMode::OnUserInitializedForDedicatedServer(const UCommonUserInfo* UserInfo, bool bSuccess, FText Error, ECommonUserPrivilege RequestedPrivilege, ECommonUserOnlineContext OnlineContext)
{
	...
	if (GameInstance)
	{
		...
		if (bSuccess)
		{
			...
			UE_LOG(LogFlipsideExperience, Log, TEXT("Dedicated server online login succeeded, starting online server"));
		...
		else
		{
			...
			UE_LOG(LogFlipsideExperience, Log, TEXT("Dedicated server online login failed, starting LAN-only server"));
...
void ALyraGameMode::OnMatchAssignmentGiven(FPrimaryAssetId ExperienceId, const FString& ExperienceIdSource)
{
	if (ExperienceId.IsValid())
	{
		UE_LOG(LogFlipsideExperience, Log, TEXT("Identified experience %s (Source: %s)"), *ExperienceId.ToString(), *ExperienceIdSource);
	...
	{
		UE_LOG(LogFlipsideExperience, Error, TEXT("Failed to identify experience, loading screen will stay up forever"));
	}
}

void ALyraGameMode::OnExperienceLoaded(const UFlipsideExperienceDefinition* CurrentExperience)
{
	...
	if (UClass* PawnClass = GetDefaultPawnClassForController(NewPlayer))
	{
		if (APawn* SpawnedPawn = GetWorld()->SpawnActor<APawn>(PawnClass, SpawnTransform, SpawnInfo))
		{
			if (ULyraPawnExtensionComponent* PawnExtComp = ULyraPawnExtensionComponent::FindPawnExtensionComponent(SpawnedPawn))
			{
				...
				{
					UE_LOG(LogFlipside, Error, TEXT("Game mode was unable to set PawnData on the spawned pawn [%s]."), *GetNameSafe(SpawnedPawn));
				}
			}
		...
		{
			UE_LOG(LogFlipside, Error, TEXT("Game mode was unable to spawn Pawn of class [%s] at [%s]."), *GetNameSafe(PawnClass), *SpawnTransform.ToHumanReadableString());
		}
	}
	else
	{
		UE_LOG(LogFlipside, Error, TEXT("Game mode was unable to spawn Pawn due to NULL pawn class."));
	}
...
void ALyraGameMode::FailedToRestartPlayer(AController* NewPlayer)
{
	...
	if (UClass* PawnClass = GetDefaultPawnClassForController(NewPlayer))
	{
		if (APlayerController* NewPC = Cast<APlayerController>(NewPlayer))
		{
			...
			{
				UE_LOG(LogFlipside, Verbose, TEXT("FailedToRestartPlayer(%s) and PlayerCanRestart returned false, so we're not going to try again."), *GetPathNameSafe(NewPlayer));
			}
		}
		else
		{
	...
	{
		UE_LOG(LogFlipside, Verbose, TEXT("FailedToRestartPlayer(%s) but there's no pawn class so giving up."), *GetPathNameSafe(NewPlayer));
	}
}
