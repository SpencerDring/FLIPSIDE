...
FPrimaryAssetId ALyraWorldSettings::GetDefaultGameplayExperience() const
{
	...
	if (!DefaultGameplayExperience.IsNull())
	{
		...
		if (!Result.IsValid())
		{
			UE_LOG(LogFlipsideExperience, Error, TEXT("%s.DefaultGameplayExperience is %s but that failed to resolve into an asset ID (you might need to add a path to the Asset Rules in your game feature plugin or project settings"),
