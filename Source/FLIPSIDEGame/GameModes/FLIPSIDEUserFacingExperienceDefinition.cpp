...
#include "FLIPSIDEUserFacingExperienceDefinition.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(FLIPSIDEUserFacingExperienceDefinition)

namespace FLIPSIDE::Experience
{
	UE_DEFINE_GAMEPLAY_TAG_STATIC(TAG_Platform_Trait_ReplaySupport, "Platform.Trait.ReplaySupport");
};

UCommonSession_HostSessionRequest* UFlipsideUserFacingExperienceDefinition::CreateHostingRequest() const
{
	...
	if (ICommonUIModule::GetSettings().GetPlatformTraits().HasTag(FLIPSIDE::Experience::TAG_Platform_Trait_ReplaySupport.GetTag()))
	{
		if (bRecordReplay)
		{
