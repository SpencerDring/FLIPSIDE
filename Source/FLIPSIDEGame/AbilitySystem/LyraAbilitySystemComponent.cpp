		...
		if (LyraAbilityCDO->GetInstancingPolicy() != EGameplayAbilityInstancingPolicy::NonInstanced)
		{
			...
			for (UGameplayAbility* AbilityInstance : Instances)
			{
				...
				if (ShouldCancelFunc(LyraAbilityInstance, AbilitySpec.Handle))
				{
					...
					{
						UE_LOG(LogFlipsideAbilitySystem, Error, TEXT("CancelAbilitiesByFunc: Can't cancel ability [%s] because CanBeCanceled is false."), *LyraAbilityInstance->GetName());
					}
				}
			}
		}
...
void ULyraAbilitySystemComponent::AddAbilityToActivationGroup(ELyraAbilityActivationGroup Group, ULyraGameplayAbility* LyraAbility)
{
	...
	{
		UE_LOG(LogFlipsideAbilitySystem, Error, TEXT("AddAbilityToActivationGroup: Multiple exclusive abilities are running."));
	}
}
...
{
	const TSubclassOf<UGameplayEffect> DynamicTagGE = UFlipsideAssetManager::GetSubclass(UFlipsideGameData::Get().DynamicTagGameplayEffect);
	if (!DynamicTagGE)
	{
		UE_LOG(LogFlipsideAbilitySystem, Warning, TEXT("AddDynamicTagGameplayEffect: Unable to find DynamicTagGameplayEffect [%s]."), *UFlipsideGameData::Get().DynamicTagGameplayEffect.GetAssetName());
		return;
	}
	...
	if (!Spec)
	{
		UE_LOG(LogFlipsideAbilitySystem, Warning, TEXT("AddDynamicTagGameplayEffect: Unable to make outgoing spec for [%s]."), *GetNameSafe(DynamicTagGE));
		return;
	}
...
void ULyraAbilitySystemComponent::RemoveDynamicTagGameplayEffect(const FGameplayTag& Tag)
{
	...
	if (!DynamicTagGE)
	{
		UE_LOG(LogFlipsideAbilitySystem, Warning, TEXT("RemoveDynamicTagGameplayEffect: Unable to find gameplay effect [%s]."), *UFlipsideGameData::Get().DynamicTagGameplayEffect.GetAssetName());
		return;
	}
